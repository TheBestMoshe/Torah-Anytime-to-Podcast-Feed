...
resource "google_cloud_run_service" "beta-rss-service" {
  name     = "beta-rss-service"
  location = var.region
  ...
  template {
    spec {
      containers {
        # image = "gcr.io/google-samples/hello-app:2.0"
        image = "gcr.io/${var.project_name}/main_server:beta"
      }
      timeout_seconds = 50
...
output "beta-url" {
  value = google_cloud_run_service.beta-rss-service.status[0].url
}
